 ...
 * 言い出しっぺ
 * 社会人2年目
 * 普段は基本的にはインフラ担当(主にミドルウェアとかIDCFクラウドとかAWSとか)
 * CIが好き・自動化好き
 * 好きなツールはChef
* Mさん(twitterやってない)
...
#### 直前
* 諸事情により@dekokunがあまり稼働出来ないことに

#### 1日目
* 接続できず問い合わせ
* 仕事後に軽くやる
* とりあえずPerconaインストールして本番ベンチ回す -> 動かぬ
* 諦める
* ローカルベンチようにtpcc_loadでデータ作成を走らせて帰る

#### 2日目
* tpcc_load失敗していたことに気付き絶望
* 朝に昨日ベンチ回らなかったけどどうだろうと思いつつ本番ベンチ回す -> 動かぬ
* 昼にも同じように回す -> 動かぬ
* @dekokunがgitの設定入れてくれた
* 仕事後に軽く設定しつつ、ローカルベンチ回してみた
* 家に帰って本番ベンチ回してみたら動いた -> 10681.066

#### 3日目
* 「ベンチマークが実行できません」が発生
* ひたすら悩みながら色々やったが解決せず
* 「ベンチマークが実行できません。」と表示される方へのメールが届きまさか・・・と思い問い合わせ
* 夜中もがんばてった
* service mysql stop/startが行われることがわかり原因判明
 * initファイル置いてなかった

#### 4日目
* 「第一陣　競技期間延長のお知らせ」が届き完全にやる気がなくなる
* 延長戦で本気出すと誓う
* そのまま終了
...
#### 直前
* @dekokunが離脱宣言
* 三兎を追わずmariadb一本で行くことにした

#### 1日目
* とりあえず元の設定に戻す
* initファイル用意
 * ここで実はinitファイルにバグを仕込んでしまっていた
* 本番ベンチ回す
 * データディレクトリcp完了・起動を確認したら以下のようなerorr_log大量発生
 * ??????は数値
 ```
 InnoDB: Error: page ?????? log sequence number ??????
 InnoDB: is in the future! Current system log sequence number ??????.
 ```
* またベンチを回すこととの戦いに没頭
* 起動時にib_logファイルがないことに気づく
 * 上記のエラーは正しいib_logファイルがないと治らん!
 * initファイル内に/nfs_mountからib_logファイルをcpしてくるようにした
 * そして起動しすぐに停止し、ib_logfileのサイズを変えたバージョンのmy.cnfを読み込むようなinitファイルに変更
 * つまり[service mysql start]で起動->停止->起動という処理を組み込んだ
* それでも本番ベンチ回らん
* 一旦諦めてローカルベンチしつつチューニング
* @gotyoooo徹夜
 * 試しにxfsからext4に変更したらローカルベンチながら非常に良い結果が出る

#### 2日目
* レギュレーション上どうかわからんinit内で事前に準備したバッファプールダンプファイルを読み込ませるようにした。
* mariadbのデータディレクトリは罠なんじゃないかと考えて、Perconaに変更
* さすがにこのまま本番ベンチ回らんのはまずいと思い、朝に運営の方に問い合わせ
* 送っていただいたエラーログをみて、起動後もinitファイルが終了していないことに気づく
 * 踏み台サーバからssh経由でコマンド実行したらそうなった
 * ローカルでしかserviceコマンドを打っていなかったから標準出力から抜けていないことに気づけなかった…
* 運営の皆さんに罪悪感を感じつつ直して本番ベンチ実行
 * 成功!しかも延長戦2位じゃないか!
 * @gotyoooo,MさんテンションMAX
 * しかもswap使ってたし、もう少しあげられるじゃないかとか考えた
 * ただこの時点で時間的に残り2回しか本番ベンチ回せないことに絶望
* かなり挑戦的な設定で本番ベンチ…惨敗
* 時間がなく仕方ないので以下だけ変えて本番ベンチ
 * swapでないようにチューニング
 * バッファプールダンプ読み込みしないようにした(あとで後悔)
* 競技時間終了
* 結果スコア更新ならず
